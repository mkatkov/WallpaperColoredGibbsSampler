classdef Exp4Analysis < handle
    ...
    methods
        function obj= Exp4Analysis( nick, mode )
            ...
            fname= sprintf( '../ExpData/%s_%s.mat', metaCl.Name, '' );
            fprintf( '%s\n', fname )
            if exist( fname, 'file' ),
                obj.data= load(fname);
            else
...
%                obj.presentation.runTrial(  obj.data.betaIdx(obj.data.curBeta), obj.data.curImg );
                ...
                obj. presentation. drawStimFrame( obj.data.curBeta, obj.data.curImg , 1);
                cImg{cImgIdx}= Screen( 'GetImage', obj.presentation.win );
                cImgIdx=cImgIdx+1;
                Screen('Flip', obj.presentation.win );
                
                if obj.data.curBeta >= 100,
                    ...
                    if ~isempty( trData),
                        obj.presentation.trialData= trData;
                        [obj.data.curBeta, obj.data.curImg]= ind2sub( [100, 10], numel(trData. report)+1 );
                    else
