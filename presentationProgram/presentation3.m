...
classdef presentation3 < handle
    ...
    methods
        ...
        function drawStimFrame( obj, betaIdx, imgIdx, quadrant, nFrames )
            ...
            %quadrant= randi(4);
            obj. drawTexture( gr.data, obj. texInd, texRect, k+1, screenWidth, screenHeight );
            end
...
%            end
obj. fixationColor= [1 1 1]* rand()*256;
            obj.drawFixation();
            for k=1:nFrames,
 ...
 %           obj.trialData. imgIdx(end+1)= imgIdx;
        ...
        function prepareNormalScreen( obj, mode )
            AssertOpenGL;
            ...
            if mode,
             [obj. win, obj.winRect]= Screen('OpenWindow', screenNumber, [0.25 0.25 0.25]*256 );   % open screen number x with a gray color background.
            else
            [obj. win, obj.winRect]= Screen('OpenWindow', screenNumber, [0.25 0.25 0.25]*256, [0 0 1200 800]);   % open screen number x with a gray color background.
            end
        end
        ...
        function drawFixation( obj )
            screenWidth=obj. winRect(3)-obj.winRect(1);
            screenHeight=obj. winRect(4)-obj.winRect(2);
            Screen( 'DrawDots', obj.win, [screenWidth; screenHeight]/2, 3, obj. fixationColor );
        end
...
%            obj.trialData. trialOnset(end+1)= secs;
            ...
            betasIdxs= round(linspace( 429, 487, 40 ) );
            forward= false; 
            
            for k=1:30,
                for quad=[1 1 1 1]%[1 2 4 3],
                    obj. drawStimFrame( betasIdxs(betaIdx), imgIdx, quad, 15 ); % 250 ms
                    Screen('Flip', obj.win );
                    if forward,
                        if betaIdx >= 40,
                            forward= false;
