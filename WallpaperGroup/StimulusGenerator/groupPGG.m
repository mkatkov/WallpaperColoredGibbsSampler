classdef groupPGG < SquareLattice
    
    methods
        function obj= groupPGG()
        ...
        function gs= createUpdateGroups( obj, beta )
            ...
            % glide reflection over short axis
            for k2=7:-1:0,
                for k1=3:-1:0,
                    sidx2( (1:8)+8+k1*16, (1:4)+4+k2*8 ) = idx( (1:8)  +k1*16, (4:-1:1)  +k2*8 );
                    sidx2( (1:8)+  k1*16, (1:4)+4+k2*8 ) = idx( (1:8)+8+k1*16, (4:-1:1)  +k2*8 );
                    sidx2( (1:8)+8+k1*16, (1:4)  +k2*8 ) = idx( (1:8)  +k1*16, (4:-1:1)+4+k2*8 );
                    sidx2( (1:8)+  k1*16, (1:4)  +k2*8 ) = idx( (1:8)+8+k1*16, (4:-1:1)+4+k2*8 );
                end
            end
%             for k=7:-1:0,
...
%                 end
        ...
        function data= getGlideReflectionLong()
            data= obj.data;
            for k2=7:-1:0,
                for k1=3:-1:0,
        ...
        function data= getGlideReflectionShort()
            data= obj.data;
            for k2=7:-1:0,
                for k1=3:-1:0,
                    sidx2( (1:8)+8+k1*16, (1:4)+4+k2*8 ) = idx( (1:8)  +k1*16, (4:-1:1)  +k2*8 );
...
%             end            
            ...
            for grIdx= 1:8,
                pls=[];
                for k=0:2:7,
                    pl= idx( mod( (0:15)+   16*(k+mod(grIdx-1, 4))+8, 64)+ 1 , mod((1:8)+ (k+floor((grIdx-1)/2))*8 +3, 64)+1);
                    pls= [pls; pl(:)];
                    %pl= idx( mod( 32+ (0:15)+   16*(k+grIdx)+8, 64)+ 1 , mod( (1:8)+ k*8+3, 64)+1 );
                    %pls= [pls; pl(:)];
                end
                gs. updateGroups(grIdx). places= pls;
