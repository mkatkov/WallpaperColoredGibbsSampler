classdef groupP31M < HexagonalLattice
    properties
        symm;   % a set of symmetry transformations of a cell
    end
    methods
        function obj= groupP31M()
...
%         end
        function ir= getInteractionRules( obj, beta )
            ...
            id= Potential();
            id.U = log(eye(3)+1e-1)*beta;
            id2= Potential();
            id2.U = log(eye(3)+1e-1)*beta;
        ...
        function ir= getInteractionRules2( obj, betas )
            ...
            %first cell should be replicated
            id= Potential();
            id.U = log(eye(3)+1e-1)*betas(1);
            id2= Potential();
            id2.U = log(eye(3)+1e-1)*betas(2);
            id3= Potential();
            id3.U = log(eye(3)+1e-1)*betas(3);
            ids= {id, id2, id3};
